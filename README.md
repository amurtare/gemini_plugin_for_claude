...
| 항목 | 조치 |
|------|------|
| Path Traversal | threadId를 UUID 형식으로 검증하여 임의 경로 접근 차단 |
| Shell Injection | `shell:false` + Node.js 직접 실행으로 셸 메타문자 주입 방지 |
| 프롬프트 인젝션 | 대화 히스토리의 role 태그를 새니타이즈하여 모델 조작 방지 |
| 환경변수 누출 | 자식 프로세스에 `GOOGLE_*`/`GEMINI_*` 등 필요한 변수만 전달 |
| 소켓 권한 | Unix socket 파일 퍼미션 `0600` (소유자만 접근) |
| 히스토리 DoS | 스레드당 최대 500턴 제한, 초과 시 자동 트림 |
| ACP 파일 접근 | workspace 경계 밖 파일 읽기/쓰기 차단 |
| RPC 타임아웃 | ACP 요청에 120초 타임아웃 적용, 무응답 시 자동 해제 |
| Handler 리크 | ACP update handler를 턴 종료 시 자동 해제 |
| 에러 정보 노출 | 구조화된 에러 스키마로 내부 경로/토큰 노출 최소화 |

### 설계 원칙

- **로컬 우선**: 세션 데이터는 로컬 디스크에만 저장. 클라우드 세션 resume 미사용
- **최소 권한**: ACP 프로세스는 기본 read-only. write 요청 시에만 workspace 범위 내 파일 접근 허용
- **폴백 안전성**: ACP 실패 시 CLI spawn으로 자동 폴백, 어느 경로든 동일한 보안 정책 적용
